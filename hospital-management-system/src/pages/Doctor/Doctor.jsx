...
export default function DoctorDashboard() {
    ...
    // ✅ FIXED - Today's appointments filtering
    const todayAppts = appointments.filter(a => {
        // Check both 'date' and 'appointment_date' fields
        const aDate = a.date || a.appointment_date || "";
        // Handle both "YYYY-MM-DD" and "YYYY-MM-DDTHH:MM:SS" formats
        const dateOnly = aDate.split("T")[0];
        return dateOnly === today;
    });

    const filtAppts = appointments.filter(a => {
        const ms = apptFilter==="All" || a.status===apptFilter;
        const md = !apptDate || a.date===apptDate || a.appointment_date===apptDate || (a.date||a.appointment_date||"").split("T")[0]===apptDate;
        return ms && md;
    });

    // ✅ FIXED - Build unique patient list from appointments (single source of truth)
    const derivedPatients = (() => {
        ...
        appointments
          ...
          .forEach(a => {
            ...
            const aDate = (a.date || a.appointment_date || "").split("T")[0];
                                        ...
                                        <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                                            ...
                                            <div style={{flex:1,minWidth:0}}>
                                                ...
                                                <div style={{fontSize:12,color:"#64748b",marginBottom:4}}>
                                                    📅 {fmtDate(a.date || a.appointment_date)} · ⏰ {fmtTime(a.time_slot)}
                                                ...
                                                {lastAppt&&(
                                                    <div style={{background:lastAppt.visit_type==="online"?"#eff6ff":"#f0fdfb",borderRadius:10,padding:"8px 12px",border:`1px solid ${lastAppt.visit_type==="online"?"#bfdbfe":"#ccfbf1"}`}}>
                                                        ...
                                                        <div style={{fontSize:12,fontWeight:700,color:"#0f172a",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                                            <span>{fmtDate(lastAppt.date||lastAppt.appointment_date)} {fmtTime(lastAppt.time_slot)}</span>
